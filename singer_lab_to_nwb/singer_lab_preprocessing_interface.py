...
from update_task_conversion_utils import get_module, get_spikegadgets_electrode_metadata
...
class SingerLabPreprocessingInterface(BaseDataInterface):
    ...
    def run_conversion(self, nwbfile: NWBFile, metadata: dict, stub_test: bool = False, skip_decomposition: bool=False):
        ...
        lfp_obj = get_electrical_series(channel_dirs, mat_loader, rec_files, 'eeg', elec_table_region)
...
def get_electrical_series(channel_dirs, mat_loader, rec_files, signal_name, elec_table_region):
    signal_dict = dict(eeg=dict(name='LFP',
                                descript='local field potential data generated by singer lab preprocessing pipeline'),
                       raweeg=dict(name='raw_electrical_series',
                                   descript='raw data from singer lab preprocessing pipeline data structures'))
    ...
    data = MultiFileArrayIterator(channel_dirs, signal_name, mat_loader, recs, num_samples)
    ...
    if band_name == 'LFP':  # if LFP, wrap in the LFP specific data object
    ...
    for band in signal_bands:
        ...
        for ind, file in enumerate(filenames):
            ...
            for field in fields:
                ...
                if field not in ['excluderipples']:
